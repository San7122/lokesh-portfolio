...
function GridCanvas() {
  ...
  useEffect(() => {
    ...
    // Isometric container: dark solid body + glowing orange edges
    const isoBox = (sx, sy, W, D, H, glow) => {
      if (W < 2 || H < 2) return;
      ...
      const face = (pts, darkFill, edgeA) => {
        ctx.beginPath(); ctx.moveTo(pts[0][0],pts[0][1]);
        pts.slice(1).forEach(p=>ctx.lineTo(p[0],p[1])); ctx.closePath();
        ctx.fillStyle = darkFill; ctx.fill();
        ctx.strokeStyle = `rgba(245,158,11,${edgeA})`; ctx.lineWidth = 0.8; ctx.stroke();
      };
      // Dark metallic fills, bright edges — premium container look
      face([fl,bl,tbl,tl], 'rgba(12,8,4,0.85)',  glow * 0.5);  // left (shadow)
      face([fl,fr,tr, tl], 'rgba(18,11,5,0.80)',  glow * 0.85); // front
      face([tl,tr,tbr,tbl],'rgba(28,17,6,0.70)',  glow);        // top (lightest)

      // Container ribs
      const ribs = Math.max(2, Math.floor(W / 14));
      for (let i = 1; i < ribs; i++) {
        const f = i / ribs;
        ctx.beginPath();
        ctx.moveTo(fl[0]+(fr[0]-fl[0])*f, tl[1]+(tr[1]-tl[1])*f);
        ctx.lineTo(fl[0]+(fr[0]-fl[0])*f, fl[1]+(fr[1]-fl[1])*f);
        ctx.strokeStyle = `rgba(245,158,11,${glow*0.3})`; ctx.lineWidth=0.35; ctx.stroke();
      }
    };

    // Draw one container yard: cols wide × rows tall, floating gently
    const yard = (baseX, baseY, cols, rows, W, D, H, glow, floatY) => {
      const sx = W*C + D*C + 3; // col step X
      const sy = W*S - D*S;     // col step Y (iso drift)
      for (let col = 0; col < cols; col++)
        for (let row = 0; row < rows; row++)
          isoBox(baseX + col*sx, baseY + col*sy - row*H + floatY, W, D, H, glow);
    };

    // Fixed (deterministic) logistics network
    const mkNet = () => {
      const hubs = [
        { x:w*0.22, y:h*0.28, ph:0.0 },
        { x:w*0.42, y:h*0.18, ph:1.1 },
        { x:w*0.58, y:h*0.82, ph:2.2 },
        { x:w*0.78, y:h*0.72, ph:3.3 },
        { x:w*0.38, y:h*0.55, ph:0.7 },
        { x:w*0.62, y:h*0.45, ph:1.8 },
      ];
      const routes = [
        { x1:hubs[0].x,y1:hubs[0].y, cx:w*0.32,cy:h*0.10, x2:hubs[1].x,y2:hubs[1].y },
        { x1:hubs[1].x,y1:hubs[1].y, cx:w*0.52,cy:h*0.30, x2:hubs[5].x,y2:hubs[5].y },
        { x1:hubs[5].x,y1:hubs[5].y, cx:w*0.70,cy:h*0.58, x2:hubs[3].x,y2:hubs[3].y },
        { x1:hubs[4].x,y1:hubs[4].y, cx:w*0.50,cy:h*0.72, x2:hubs[2].x,y2:hubs[2].y },
        { x1:hubs[0].x,y1:hubs[0].y, cx:w*0.40,cy:h*0.44, x2:hubs[4].x,y2:hubs[4].y },
        { x1:hubs[2].x,y1:hubs[2].y, cx:w*0.68,cy:h*0.62, x2:hubs[3].x,y2:hubs[3].y },
      ];
      const pkgs = routes.map(r => ({ r, t:Math.random(), sp:0.0007+Math.random()*0.0009, trail:[] }));
      return { hubs, routes, pkgs };
    };
    ...
    const draw = () => {
      ...
      const { hubs, routes, pkgs } = net;

      // Subtle depth lines (not a grid — just parallel diagonals for depth)
      ctx.lineWidth=0.35; ctx.strokeStyle="rgba(245,158,11,0.014)";
      for (let x = -h; x <= w+h; x += 80) {
        ctx.beginPath(); ctx.moveTo(x,0); ctx.lineTo(x+h*0.6,h); ctx.stroke();
      }

      // ── BOTTOM-LEFT YARD (origin terminal) ──
      const W=48, D=22, H=20;
      const cols = w > 1200 ? 6 : w > 900 ? 5 : 4;
      const floatA = Math.sin(time*0.35)*2.5;
      // Anchor bottom-left: baseY high enough that 3-row stack fits above h
      yard(w*0.012, h*0.92, cols, 3, W, D, H, 0.38, floatA);

      // ── TOP-RIGHT YARD (destination terminal) ──
      const colSX = W*C + D*C + 3;
      const rx = w - cols*colSX - w*0.03;
      const floatB = Math.sin(time*0.35+1.6)*2.5;
      yard(rx, h*0.10, cols, 3, W, D, H, 0.32, floatB);

      // ── Dashed route lines ──
      ctx.setLineDash([5,13]); ctx.lineWidth=1;
      routes.forEach(r=>{
        ctx.beginPath(); ctx.moveTo(r.x1,r.y1);
        ctx.quadraticCurveTo(r.cx,r.cy,r.x2,r.y2);
        ctx.strokeStyle="rgba(245,158,11,0.09)"; ctx.stroke();
      });
      ctx.setLineDash([]);

      // ── Hub nodes (warehouse/depot markers) ──
      hubs.forEach(hub=>{
        const p = Math.sin(time*1.3+hub.ph)*0.5+0.5;
        ctx.beginPath(); ctx.arc(hub.x,hub.y,3+p*6,0,Math.PI*2);
        ctx.strokeStyle=`rgba(245,158,11,${0.07+p*0.11})`; ctx.lineWidth=1; ctx.stroke();
        ctx.beginPath(); ctx.arc(hub.x,hub.y,3,0,Math.PI*2);
        ctx.fillStyle=`rgba(245,158,11,${0.25+p*0.28})`; ctx.fill();
      });

      // ── Moving packages (mini iso boxes + glow trail) ──
      pkgs.forEach(pkg=>{
        pkg.t += pkg.sp;
        ...
        pkg.trail.forEach((tp,ti)=>{
          ...
          ctx.beginPath(); ctx.arc(tp.x,tp.y,1.7*a,0,Math.PI*2);
          ctx.fillStyle=`rgba(252,211,77,${a*0.32})`; ctx.fill();
        });
        isoBox(pos.x-3.5, pos.y, 8, 5, 6, 0.6);
      });
    ...
    draw();
    const onResize = () => { resize(); net = mkNet(); };
...
function ScrollBar() {
  ...
  return <div style={{ position: "fixed", top: 0, left: 0, height: 3, width: `${p*100}%`, background: "linear-gradient(90deg,#f59e0b,#ff4444,#ffd700)", zIndex: 9999 }} />;
}
...
function Typer({ texts }) {
  ...
  return <span>{txt}<span style={{ borderRight: "2px solid #f59e0b", marginLeft: 2, animation: "blink 1s step-end infinite" }}>&nbsp;</span></span>;
}
  ...
  return (
    <div ref={ref}
      onMouseMove={e => {
        ...
        setGlow(`radial-gradient(circle at ${x}px ${y}px, rgba(245,158,11,0.1), transparent 60%)`);
      }}
      ...
      style={{
        ...
        border: "1px solid rgba(245,158,11,0.1)", borderRadius: 16, backdropFilter: "blur(6px)", ...style,
      }}
    >{children}</div>
  );
}
  ...
  return (
    <svg viewBox="0 0 100 80" style={{ width: "100%", maxWidth: 400, height: "auto", margin: "0 auto", display: "block" }}>
      ...
      {hubs.map((h, i) => hubs.slice(i+1).filter((_, j) => j < 3).map((h2, j) => (
        <line key={`${i}-${j}`} x1={h.x} y1={h.y} x2={h2.x} y2={h2.y} stroke="rgba(245,158,11,0.12)" strokeWidth="0.3" />
      )))}
      {/* hub dots */}
      {hubs.map((h, i) => (
        <g key={i}>
          {i === pulse && <circle cx={h.x} cy={h.y} r="4" fill="none" stroke="#f59e0b" strokeWidth="0.3" opacity="0.5">
            <animate attributeName="r" from="2" to="6" dur="1s" repeatCount="indefinite" />
            <animate attributeName="opacity" from="0.6" to="0" dur="1s" repeatCount="indefinite" />
          </circle>}
          <circle cx={h.x} cy={h.y} r="1.5" fill="#f59e0b" opacity={i === pulse ? 1 : 0.4} />
        </g>
      ))}
    </svg>
  );
...
export default function Portfolio() {
  ...
  const accent = "#f59e0b";
  const accentGlow = { color: accent, textShadow: `0 0 25px rgba(245,158,11,0.4), 0 0 50px rgba(245,158,11,0.12)` };
  ...
  const experiences = [
    { role: "Hub Manager – Cluster Operations", company: "Pronto", location: "New Delhi", period: "Oct 2025 – Present", color: "#f59e0b",
      highlights: [
  ...
  return (
    <div style={{ fontFamily: bf, background: "#0b0e14", color: "#c4c0b8", minHeight: "100vh", overflowX: "hidden" }}>
      <style>{`
        ...
        @keyframes pulse{0%,100%{box-shadow:0 0 15px rgba(245,158,11,0.15)}50%{box-shadow:0 0 35px rgba(245,158,11,0.3)}}
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        ::-webkit-scrollbar-thumb{background:linear-gradient(#f59e0b,#ff4444);border-radius:3px}
        a{color:#f59e0b;text-decoration:none;transition:all 0.3s}
      ...
      <div style={{ position:"fixed",top:"10%",right:"10%",width:250,height:250,borderRadius:"50%",background:"radial-gradient(circle,rgba(245,158,11,0.03),transparent 70%)",transform:`translate(${px*-25}px,${py*-25}px)`,zIndex:0,pointerEvents:"none" }} />
      ...
      <section ref={heroRef} style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",zIndex:1,padding:"40px 24px",perspective:1200,...heroSt }}>
        <div style={{ maxWidth:900,textAlign:"center",transformStyle:"preserve-3d" }}>
          {/* Status indicator */}
          <div style={{ display:"inline-flex",alignItems:"center",gap:8,padding:"8px 20px",borderRadius:30,border:"1px solid rgba(245,158,11,0.2)",background:"rgba(245,158,11,0.05)",marginBottom:28,fontSize:13,fontFamily:sf,color:"#f59e0b" }}>
          ...
          <div style={{ perspective:800,marginBottom:32 }}>
            ...
            <div style={{ width:120,height:16,margin:"-8px auto 0",borderRadius:"50%",background:"rgba(245,158,11,0.06)",filter:"blur(10px)" }} />
          </div>
          ...
          <h1 style={{ fontFamily:hf,fontSize:"clamp(52px,10vw,100px)",fontWeight:400,lineHeight:0.95,color:"#f0ece4",letterSpacing:4,textTransform:"uppercase",marginBottom:20,transform:"translateZ(20px)" }}>
            LOKESH<br/>
            <span style={{ background:"linear-gradient(135deg,#f59e0b,#ff4444,#ffd700)",backgroundSize:"200% 200%",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"gradShift 4s ease infinite" }}>KUMAR</span>
          </h1>
          ...
          <div style={{ display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap",transform:"translateZ(5px)" }}>
            <a href="#contact" style={{ padding:"13px 32px",borderRadius:8,fontSize:14,fontWeight:700,fontFamily:sf,background:"linear-gradient(135deg,#f59e0b,#ff4444)",color:"#0b0e14",transition:"all 0.35s",textTransform:"uppercase",letterSpacing:1,display:"inline-block" }}
              onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow="0 12px 40px rgba(245,158,11,0.3)"}}
              onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow=""}}
            >Contact Me</a>
            <a href="#experience" style={{ padding:"13px 32px",borderRadius:8,fontSize:14,fontWeight:700,fontFamily:sf,border:"1px solid rgba(245,158,11,0.3)",color:"#f59e0b",textTransform:"uppercase",letterSpacing:1,transition:"all 0.35s",display:"inline-block" }}
              onMouseEnter={e=>{e.currentTarget.style.background="rgba(245,158,11,0.08)";e.currentTarget.style.transform="translateY(-3px)"}}
          ...
          {Object.entries(skills).map(([cat,items],ci) => (
            <Card3D key={ci} style={{ padding:28 }}>
              ...
              <div style={{ display:"flex",flexWrap:"wrap",gap:8 }}>
                {items.map((s,si) => (
                  <span key={si} style={{ display:"inline-block",padding:"6px 14px",borderRadius:6,fontSize:12,fontFamily:sf,background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.12)",color:"#b0aca4",cursor:"default",transition:"all 0.3s",animation:`float 3s ease ${si*0.2}s infinite` }}
                    onMouseEnter={e=>{e.currentTarget.style.background="rgba(245,158,11,0.2)";e.currentTarget.style.color="#fff";e.currentTarget.style.transform="translateY(-3px) scale(1.05)";e.currentTarget.style.boxShadow="0 6px 20px rgba(245,158,11,0.15)"}}
                    onMouseLeave={e=>{e.currentTarget.style.background="rgba(245,158,11,0.06)";e.currentTarget.style.color="#b0aca4";e.currentTarget.style.transform="";e.currentTarget.style.boxShadow=""}}
                  >{s}</span>
                ))}
              </div>
            </Card3D>
      ...
      <section id="experience" ref={expRef} style={{ ...container,...expSt,padding:"100px 24px" }}>
        ...
        <div style={{ position:"relative",paddingLeft:40 }}>
          {/* Timeline */}
          <div style={{ position:"absolute",left:10,top:0,bottom:0,width:2,background:"linear-gradient(180deg,#f59e0b,#ff4444,#ffd700,transparent)",backgroundSize:"100% 200%",animation:"gradShift 6s ease infinite" }} />
            ...
            return (
              <div key={i} ref={ref} style={{ marginBottom:40,position:"relative",...style }}>
                ...
                <Card3D style={{ padding:28,position:"relative",overflow:"hidden" }} depth={6}>
                  {/* Scanline effect */}
                  <div style={{ position:"absolute",top:"-100%",left:0,right:0,height:"50%",background:"linear-gradient(transparent, rgba(245,158,11,0.02), transparent)",animation:"scanline 8s linear infinite",pointerEvents:"none" }} />
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:24 }}>
          {education.map((ed,i) => (
            <Card3D key={i} style={{ padding:30 }}>
              <div style={{ fontFamily:sf,fontSize:11,color:accent,fontWeight:700,marginBottom:10,padding:"4px 12px",background:"rgba(245,158,11,0.06)",borderRadius:8,display:"inline-block",letterSpacing:1 }}>{ed.year}</div>
      ...
      <section id="contact" ref={contactRef} style={{ ...container,...contactSt,padding:"100px 24px 70px",textAlign:"center" }}>
        ...
        <div style={{ display:"flex",justifyContent:"center",gap:16,flexWrap:"wrap" }}>
          {[
            ...
            <a key={i} href={l.href} style={{ padding:"12px 28px",borderRadius:8,fontSize:13,fontWeight:700,fontFamily:sf,border:"1px solid rgba(245,158,11,0.2)",color:"#f59e0b",textTransform:"uppercase",letterSpacing:2,transition:"all 0.35s" }}
              onMouseEnter={e=>{e.currentTarget.style.background="rgba(245,158,11,0.1)";e.currentTarget.style.transform="translateY(-3px) scale(1.05)";e.currentTarget.style.boxShadow="0 8px 30px rgba(245,158,11,0.15)"}}
